...
(function() {
    if (typeof emailjs !== 'undefined') {
        ...
        console.warn("⚠️ EmailJS no detectado. Verifica la conexión a internet.");
    }
})();
...
class ShoppingCart {
    ...
    render() {
        ...
        const checkoutBtn = document.getElementById('checkout-btn');
        const emptyMsg = document.createElement('div'); // Fallback simple
    ...
    validateForm() {
        ...
        if (name.length < 3 || name.split(' ').filter(Boolean).length < 2) {
    ...
    async sendCheckoutEmail(data) {
        ...
        try {
            ...
            console.log('✅ EmailJS enviado:', res.status, res.text);
            return true;
        } catch (error) {
    ...
    async handleOrderConfirmation() {
        ...
        if (emailSent) {
            ...
            alert('Error al enviar los datos. Por favor, revisa tu conexión e intenta de nuevo.');
        }
    }

    async processInfinitePay() {
        if (!this.orderConfirmed) {
            alert('Por favor, primero confirma y envía tus datos.');
            return;
        }
